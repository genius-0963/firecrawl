...
- **Extended SDK support**: Added Rust, Elixir, and .NET SDKs with full type definitions and async handling.
- **AI‑enhanced extraction**: Integrated OpenAI‑based `/extract` endpoint for structured JSON output and entity extraction.
- **Workflow automation**: Introduced Firecrawl Workflows for repeatable data pipelines (e.g., competitor analysis, website clone briefs).
- **Improved performance**: Updated crawling engine to reduce latency by ~15% and support higher concurrency.
- **Enhanced security**: Added optional API key rotation and rate‑limit handling for self‑hosted deployments.
